...
from lesson6.attn import ScaledDotProductAttention
...
class Decoder(nn.Module):
    ...
    def forward(self, hidden, sequence, encoder_out, encoder_mask):
        if self.training:
            embed = self.char_emb(sequence)
            out, _ = self.gru(embed, hidden)
            attn_value, attn_mtx = self.attn(encoder_out, out, encoder_out, encoder_mask)

            return self.classifier(torch.cat([out, attn_value], dim=-1))
        else:
            output_seq = []
            embed = self.char_emb(sequence)
            n_iter = 0
            ...
            while char != self.eos_id and n_iter < 20:
                ...
                attn_value, _ = self.attn(encoder_out, out, encoder_out)
                char_proba = self.classifier(torch.cat([out, attn_value], dim=-1))
...
class DateNormalizer(nn.Module):
    ...
    def forward(self, input_seq, output_seq, mask=None):
        encoder_out, hidden = self.encoder(input_seq)
        decoded = self.decoder(hidden, output_seq, encoder_out, mask)
...
for epoch in range(20):
    ...
    for step, batch in enumerate(dataloader):
        ...
        optim.zero_grad()
        predict = model(input_seq, target[:, :-1], mask.unsqueeze(-1))
    ...
    torch.save(model, './model_epoch_{}.ckpt'.format(epoch))
    # sd = torch.load('./model_epoch_8.ckpt') #должны лежать на одном девайсе
    # model.load_state_dict(sd).to(device)
